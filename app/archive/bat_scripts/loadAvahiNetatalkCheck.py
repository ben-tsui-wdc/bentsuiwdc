...
class loadAvahiNetatalkCheck(object):
    ...
    def run(self):
        ...
        try:
            ...
            netatalk_daemon = self.adb.executeShellCommand('ps | grep netatalk')[0]
            check_list1 = ['avahi-daemon: running']
            check_list2 = ['netatalk']
            if all(word in avahi_daemon for word in check_list1) and all(word in netatalk_daemon for word in check_list2):
                print 'Load Avahi and Netatalk check PASSED!!'
                testcase = TestCase(name='Load Avahi And Netatalk Check', classname='BAT', elapsed_sec=time.time()-start_time)
            else:
                raise Exception('Load Avahi and Netatalk check FAILED!!')
...
if __name__ == '__main__':
    ...
    testrun = loadAvahiNetatalkCheck(adb=adb)
    testrun.run()
