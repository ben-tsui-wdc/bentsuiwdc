...
class loadDeviceManager(object):
    ...
    def run(self):
        ...
        try:
            self.device_init()
            devmgr_daemon = self.adb.executeShellCommand('ps | grep devicemgr')[0]
            devmgr_ls_list = self.adb.executeShellCommand('ls -l /data/wd/diskVolume0/devmgr/db')[0]
            check_list1 = ['/bin/devicemgr']
            check_list2 = ['wddevice.ini']
            if all(word in devmgr_daemon for word in check_list1) and all(word in devmgr_ls_list for word in check_list2):
                print 'Load Device Manager in firmware test PASSED!!'
                testcase = TestCase(name='Load Device Manager', classname='BAT', elapsed_sec=time.time()-start_time)
            else:
                raise Exception('Load Device Manager in firmware test FAILED!!')
...
if __name__ == '__main__':
    ...
    testrun = loadDeviceManager(adb=adb)
    testrun.run()
