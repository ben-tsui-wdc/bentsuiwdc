...
class loadAfpCheck(object):
    ...
    def run(self):
        ...
        try:
            self.device_init()
            afp_daemon = self.adb.executeShellCommand('ps | grep afpd')[0]
            check_list1 = ['netatalk/sbin/afpd']
            if all(word in afp_daemon for word in check_list1):
                print 'Load AFP Service checked PASSED!!'
                testcase = TestCase(name='Load AFP Check', classname='BAT', elapsed_sec=time.time()-start_time)
            else:
                raise Exception('Load AFP Service checked FAILED!!')
