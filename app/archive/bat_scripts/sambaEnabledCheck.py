...
class sambaEnabledCheck(object):
    ...
    def run(self):
        ...
        try:
            ...
            check_list1 = ['smbd']
            if all(word in samba_daemon for word in check_list1):
                print 'Samba Enabled checked PASSED!!'
                testcase = TestCase(name='Samba Enabled Check', classname='BAT', elapsed_sec=time.time()-start_time)
            else:
                raise Exception('Samba Enabled checked FAILED!!')
...
if __name__ == '__main__':
    ...
    testrun = sambaEnabledCheck(adb=adb)
    testrun.run()
