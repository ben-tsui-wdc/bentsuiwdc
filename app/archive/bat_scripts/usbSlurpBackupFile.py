...
class usbSlurpBackupTest(object):
        ...
        try:
            ...
            for key, value in bat_chk_dic.iteritems():
                ...
                if sync_chksum == source_chksum:
                    ...
                    FAILED += 1
            if FAILED > 0:
                self.error('USB Slurp Backup Compare FAILED!! PASSED_FILES: {0}, FAILED_FILES: {1}'
                           .format(PASSED, FAILED))
            else:
                self.log.info('USB Slurp Backup Test!!!! Test PASSED!!!!')
                testcase = TestCase(name='Usb Slurp Backup Test', classname='BAT', elapsed_sec=time.time()-start_time)
...
if __name__ == '__main__':
    ...
    testrun = usbSlurpBackupTest(adb=adb, env=env)
    testrun.run()
