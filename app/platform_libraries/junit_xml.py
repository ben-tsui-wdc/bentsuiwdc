...
class TestCase(object):
    ...
    def add_attachments(self, attachments):
        """ Add attachments data to the top of stdout in test case"""
        def check_path(path):
            """
            @param path: File path, can be both rel or abs path
            @return: file aps path or False
            """
        ...
        if isinstance(attachments, (list, tuple, basestring)):
            for a in attachments if not isinstance(attachments, basestring) else [attachments]:
                check_result = check_path(a)
                if check_result:
                    output = '[[ATTACHMENT|{}]]\n'.format(check_result)
                    if self.stdout:
                        self.stdout = output + self.stdout
                    else:
                        self.stdout = output
                else:
                    print "File '{}' does not exist and will not be attached, please check the path!".format(a)
        else:
            print 'The attachments must be a list/tuple of file paths or a single entry!'

    def add_metrics(self, *metrics):
        """ Add metrics data to the top of stdout in test case"""
        def check_metric(metric):
            """ Check if the metric is in valid format
            @param metric: A list contains just 3 data: metric, value, units, e.g. ['http.upload.lan', 23, 'MB/sec']
            @return: True or False
            """
            # First, check if the metric has just 3 data in it
            if len(metric) != 3:
                print "Invalid format:{}, metric should contain 'metric', 'value', and 'units' data!".format(metric)
                return False
            # Check there are no special characters in the metric data
            re_metric = re.compile(r'^[\w\-\\/.]*$')
            for value in metric:
                if not re_metric.match('{}'.format(value)):
                    print "Invalid text in metric:{}!".format(metric)
                    return False
            return True

        def attach_output(metric):
            """ Attach metric data in stdout
            @param metric: A list contains just 3 data: metric, value, units, e.g. ['http.upload.lan', 23, 'MB/sec']
            """
            metric, value, units = metric
            output = '[[METRIC|{0}|{1}|{2}]]\n'.format(metric, value, units)
            if self.stdout:
                self.stdout = output + self.stdout
            else:
                self.stdout = output

        if len(metrics) == 1 and isinstance(metrics[0], (list, tuple)):
            # Could be a single list/tuple or a list of lists/tuples
            if any(isinstance(m, (list, tuple)) for m in metrics[0]):
                ...
                # Single list/tuple
                if check_metric(metrics[0]):
                    attach_output(metrics[0])
        elif len(metrics) == 3:
            # Single entry with 3 data arguments: metric, value, units
            if check_metric([metrics[0], metrics[1], metrics[2]]):
                attach_output(metrics)
        else:
            print 'The metrics must be a list/tuple or a single entry with metric, value, and units strings!'
