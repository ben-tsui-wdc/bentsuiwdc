...
class BTLEClient(object):
    ...
    def __init__(self, addr=None, debug=False, stream_log_level=None):
        self.addr = None
        self.device = None
        self.logger = PrintLogging()#logging.getLogger('KAT.btleclient')
    ...
    def _disconnect(self):
        ...
        try:
            _thread.start_new_thread(self.device.disconnect, ())
...
class YodaClient(BTLEClient):
    ...
    def connect(self, addr):
        ...
        self.logger.debug('Set MTU=512...')
        self.device.setMTU(512) # TODO: Need to make sure this value.
        ...
        try:
            ...
            if msg:
                if msg.endswith('00'): # Remove end char.
                    msg = msg[:-2]
                info = list(map(binascii.a2b_hex, msg.split('08')))
...
if __name__ == '__main__':
    ...
    while True:
        ...
        try:
            iteration += 1
            bt = YodaClient('00:14:EE:0C:6F:31', debug=1)
            resp = bt.get_ap_list()
            print(('AP:', resp))
            total_pass += 1
            print('PASS')
