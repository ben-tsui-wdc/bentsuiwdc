...
class BTLEClient(object):
    ...
    def __init__(self, addr=None, debug=False, stream_log_level=None):
        self.addr = None
        self.device = None
        self.logger = common_utils.create_logger(root_log='KAT.btleclient', stream_log_level=stream_log_level)
...
if __name__ == '__main__':
    bt = YodaClient('00:14:ee:0c:6e:cd', debug=1)
    bt.set_wifi('EST_5G', '1qaz2wsx')
    resp = bt.get_notify(timeout=60*5)
    print 'Device IP:', resp[0] # IP
    raise
    bt = YodaClient()
    bt.print_devices(bt.scan())
    # It may disconnect after scan.
    bt = YodaClient('00:14:ee:0c:6b:46', debug=False)
    resp = bt.get_ap_list()
    print 'AP:', resp
    bt.disconnect()
    # It may disconnect after scan, so here we disconnect.
    import time
    time.sleep(5) # No do too fast, device may not recieve request.
    bt = YodaClient('00:14:ee:0c:6b:46', debug=False)
    #resp = bt.get_notify(timeout=1)
    #print 'Device IP:', resp # None
    bt.set_wifi('EST_5G', '1qaz2wsx')
    #resp = bt.get_device_info()
    #print 'Connection info:', resp # None
    resp = bt.get_notify(timeout=60*5)
    print 'Device IP:', resp[0] # IP
    #resp = bt.get_device_ip()
    #print 'Device info:', resp # None
