...
class DesktopAppPerf(TestCase):
    ...
    def before_loop(self):
        ...
        if self.client_os == 'MAC':
            self.dest_path_in_mount_drive = "{}/{}".format(self.drive_mount_path, self.source_path.split("/")[-1])          
        elif self.client_os == 'WIN':
            self.dest_path_in_mount_drive = "{}:\\{}".format(self.drive_mount_path, self.source_path.split("\\")[-1])  

        DownAvgSpd = self.kdd_obj.file_transfer(source_path=self.source_path, dest_path=self.dest_path_in_mount_drive)  # MB/s
        print '\n    #### Pre-uploading to device is completed. ####   \n'

        # Configuration for different client_os
        if self.client_os == 'MAC':
            self.dest_path = "~/{}_desktop_app_perf".format(self.source_path.split("/")[-1])
    ...
    def before_test(self):
        # Delete testing file in client before transferring started.
    ...
    def test(self):
        ...
        print '\n    #### Start to download file from device to client ####   \n'
        DownAvgSpd = self.kdd_obj.file_transfer(source_path=self.dest_path_in_mount_drive, dest_path=self.dest_path)  # MB/s
        DownElapsT = FileSz/DownAvgSpd

        print "XX Overall Download Result " * 10
        print "FileSz: {0:.2f} MB".format(FileSz)
        print "DownElapsT: {0:.2f} seconds".format(DownElapsT)
        print "DownAvgSpd: {0:.2f} MB/s".format(DownAvgSpd)
        print  "XX Overall Download Result  " * 10
    ...
    def after_test(self):
        ...
        # Delete testing file in client after transferring completed.
        self.kdd_obj.delete_folder(self.dest_path)


    def after_loop(self):
        # Delete testing folder in target device
        self.kdd_obj.delete_folder(self.dest_path_in_mount_drive)
        ...
        if self.html_format == '1':
            html_inst.table_title_column = ['product', 'build', 'DeviceSz', 'Client', 'ConType', 'FileNum', 'FileSz', 'iteration', 'DownElapsT', 'EnvDownAvgSpd', 'DownAvgSpd',]
            html_inst.table_title_column_extend = ['DownAvgSpd']
        elif self.html_format == '2':
            html_inst.table_title_column = ['product', 'build', 'Client', 'ConType', 'iteration', 'EnvDownAvgSpd', 'DownAvgSpd',]
