# -*- coding: utf-8 -*-
""" Test case to test install app while USB slurp is running in parallel.
    https://jira.wdmv.wdc.com/browse/KAM-32495
"""
...
class InstallAppDuringUSBSlurp(TestCase):

    TEST_SUITE = 'APP_Manager_Test'
    TEST_NAME = 'KAM-32495 - Install app while USB slurp is running in parallel'
    # Popcorn
    TEST_TYPE = 'Functional'
    TEST_JIRA_ID = 'KAM-32495'
    PRIORITY = 'Critical'
    COMPONENT = 'PLATFORM'
    ISSUE_JIRA_ID = None
    ...
    def test(self):
        self.concurrent_install_app_w_usb_slurp()
        self.log.info('App({}) has been installed during USB slurp. Test PASSED !!!'.format(self.app_id))

    def concurrent_install_app_w_usb_slurp(self):
...
if __name__ == '__main__':
    parser = InputArgumentParser("""\
        *** Install app while USB slurp is running in parallel Test Script ***
        Examples: ./run.sh app_manager_scripts/concur_usb_slurp_install_app.py --uut_ip 10.92.224.68 -appid com.wdc.importapp.ibi -chkapp\
        """)
    ...
    test = InstallAppDuringUSBSlurp(parser)
    resp = test.main()
