...
class M2MTokenCheckInRestSDK(GodzillaTestCase):

    TEST_SUITE = 'Godzilla BAT'
    TEST_NAME = 'M2M Token Check In RestSDK Config'
    # Popcorn
    PROJECT = 'Godzilla'
    TEST_TYPE = 'Functional'
    TEST_JIRA_ID = 'GZA-8608'
    PRIORITY = 'Blocker'
    COMPONENT = 'PLATFORM'
    ISSUE_JIRA_ID = None
    ...
    def test(self):
        ...
        verify_string = ['m2mClientID', 'm2mClientSecret', 'rsdk']
        if not all(x in stdout for x in verify_string):
            raise self.err.TestFailure('Check M2M Token in RestSDK config failed! '
...
if __name__ == '__main__':
    ...
    test = M2MTokenCheckInRestSDK(parser)
    resp = test.main()
    if resp:
        sys.exit(0)
    sys.exit(1)
