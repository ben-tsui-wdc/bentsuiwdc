...
class WindowsSMBUpload(GodzillaTestCase):

    TEST_SUITE = 'Godzilla KPI'
    TEST_NAME = 'GZA KPI Test case - Windows SMB upload to USB drive'
    # Popcorn
    TEST_JIRA_ID = ''
    ...
    def before_test(self):
        '''
        self._umount(self.windows_mount_point, self.env.uut_ip.split(':')[0], self.specific_folder)
        time.sleep(120)
    ...
    def after_test(self):
        ...
        if self.delete_share_and_filesystem:
            # Remove SMBTestFolders of testing device by Windows mount point.
    ...
    def after_loop(self):
        ...
        if self.html_format == '2':
            html_inst.table_title_column = ['product', 'build', 'RestSDK', 'iteration', 'TotalBytes', 'TotalRealFiles', 'UpAvgSpd','TargetUpAvgSpd']
    ...
    def _umount(self, windows_mount_point, uut_ip, uut_share):
        print "\n### umount ###\n"
        cmd = 'NET USE /delete {}: /y'.format(windows_mount_point)
        result = self.XMLRPCclient(cmd)
        cmd = 'NET USE /delete \\\\{}\\{} /y'.format(uut_ip, uut_share)
        result = self.XMLRPCclient(cmd)
        self.log.info('Wait 60 seconds after unmount for avoiding Windows cache effect.' )
        time.sleep(60)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--uut_share', help='the share folder in target device which could be accessed by SMB', default='windows_upload')
