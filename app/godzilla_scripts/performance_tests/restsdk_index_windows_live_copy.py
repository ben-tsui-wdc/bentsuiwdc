...
class RestSDKIndexLiveCopy(GodzillaTestCase):

    TEST_SUITE = 'Godzilla KPI'
    TEST_NAME = 'GZA KPI Test case - RESTSDK index time for live copy'
    # Popcorn
    TEST_JIRA_ID = 'GZA-2180'
    ...
    def test(self):
        ...
        thumbnail_finish_time = thumbnail_start_time
            ...
            if not stop_check_sqlite:
                ...
                if stdout != restsdk_index_number_by_sqlite:
                    ...
                    if time.time() - restsdk_index_finish_time > waiting_time:
                        self.log.info('The number of files in sqlite doesn\'t change after consecutive {} seconds.'.format(waiting_time))
                        self.log.info('Final restsdk_index_number_by_sqlite:{}'.format(restsdk_index_number_by_sqlite))
        ...
        self.data.test_result['IndexType'] = 'live_copy' 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--uut_share', help='the share folder in target device which could be accessed by SMB', default='live_copy')
    parser.add_argument('--html_acronym_desc', help='Description which is specified.', default='')
    parser.add_argument('--target_total_indexed_file', help='target of number of indexed file', default='55')
    parser.add_argument('--target_total_created_thumbnails', help='target of number of created file', default='90')
    ...
    test = RestSDKIndexLiveCopy(parser)
    resp = test.main()
    if resp:
        sys.exit(0)
    sys.exit(1)
