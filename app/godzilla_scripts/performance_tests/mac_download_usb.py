...
class MacDownload(GodzillaTestCase):

    TEST_SUITE = 'Godzilla KPI'
    TEST_NAME = 'GZA KPI Test case - Mac download'
    # Popcorn
    TEST_JIRA_ID = ''
    ...
    def before_loop(self):
        ...
        # Just in case, remove meta data by MacOS
        stdout, stderr = self.mac_ssh.execute_cmd("ls {}/{} | grep '.*.file$' ".format(self.mac_mount_share, self.uut_share))
        if stdout:
            for element in stdout.strip().split():
                stdout, stderr = self.mac_ssh.execute_cmd('rm -fr {}/{}/{}'.format(self.mac_mount_share, self.uut_share, element))
    ...
    def before_test(self):
        ...
        self.log.info("Umount then mount again in order to avoid MacOS cache effect before downloading.")
        self.mac_ssh.unmount_folder(self.mac_mount_share, force=True)
        self.mac_ssh.mount_folder(self.transfer_protocol, self.env.ssh_ip, self.specific_folder, self.mac_mount_share)
        # Create test folder on Mac client
        self.mac_download_share = '~/mac_download_client_{}'.format(int(time.time()))
        self.mac_ssh.create_folder(self.mac_download_share)
    ...
    def test(self):
        total_bytes = self._get_file_size(file_path="{}/{}".format(self.mac_mount_share, self.uut_share))
        total_real_files = self._get_number_of_files(file_path="{}/{}".format(self.mac_mount_share, self.uut_share))
        MB_per_second = self._file_transfer(source_path="{}/{}".format(self.mac_mount_share, self.uut_share), dest_path=self.mac_download_share)
    ...
    def after_test(self):
        ...
        # Delete download share on Mac client
        self.mac_ssh.delete_folder(self.mac_download_share)


    def after_loop(self):
        # Delete the dataset folder which is uploded to GZA device
        if self.delete_share_and_filesystem:
            # Delete usb_filesystem_id
            if self.create_filesystem == 'on' and self.uut.get('firmware').startswith('5.'):
            ...
            # Remove Testfolder in GZA device from Mac client.
            self.mac_ssh.delete_folder("{}/{}".format(self.mac_mount_share, self.uut_share))

        # Umount and delete share from MacOS client
        self.mac_ssh.unmount_folder(self.mac_mount_share, force=True)
        self.mac_ssh.delete_folder(self.mac_mount_share)
        ...
        if self.html_format == '2':
            html_inst.table_title_column = ['product', 'build', 'RestSDK', 'Protocol', 'iteration', 'TotalBytes', 'TotalRealFiles', 'DownAvgSpd','TargetDownAvgSpd',]
    ...
    def _get_file_size(self, file_path=None):
    total_file_size = None
    status, stdout = self.mac_ssh.execute('du -sh {}'.format(file_path))
    total_file_size = stdout.split()[0].strip()
    return total_file_size
    '''
...
if __name__ == '__main__':
    ...
    parser.add_argument('--uut_share', help='the share folder in target device which could be accessed by SMB', default='mac_for_download_test')
