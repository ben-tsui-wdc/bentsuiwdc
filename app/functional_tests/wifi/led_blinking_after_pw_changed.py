# -*- coding: utf-8 -*-
""" Wifi Functional Test: KAM-25284, KAM-25285.
"""
...
class LedBlinkingAfterPasswordChanged(APConnect):

    TEST_SUITE = 'WIFI Functional Tests'
    TEST_NAME = 'Led Blinking After Password Changed'
    ...
    def test(self):
        ...
        try:
            self.log.test_step('Configure AP (SSID:{}, Password:{}, Security Mode:{}({}))'.format(
                self.test_ssid, temp_pw, self.test_security_mode, self.ap.security_key_mapping(self.test_security_mode))
            )

            self.set_ap(
                ssid=self.test_ssid, password=temp_pw, security_mode=self.test_security_mode, wifi_type=self.test_wifi_type, cleanup_device=False
            )
...
if __name__ == '__main__':
    parser = InputArgumentParser("""\
        *** LedBlinkingAfterPasswordChanged test on Kamino Android ***
        Examples: ./run.sh functional_tests/led_blinking_after_pw_changed.py --uut_ip 10.136.137.159\
        """)
    ...
    test = LedBlinkingAfterPasswordChanged(parser)
    resp = test.main()
    if resp:
        sys.exit(0)
    sys.exit(1)
