# -*- coding: utf-8 -*-
""" Test cases to check factory reset log uploaded when pip is off [KAM-26710]
"""
...
class FactoryResetLogUploadWhenPIPIsOff(TestCase):

    TEST_SUITE = 'Functional Test'
    TEST_NAME = 'Factory Reset Log Uploaded When PIP is Off'
    # Popcorn
    TEST_JIRA_ID = 'KAM-26710'
    ...
    def test(self):
        ...
        # Make sure pip is off
        self.make_sure_pip_off()
        ...
        # Onboarding device and turn off pip
        self.check_device_bootup()
    ...
    def make_sure_pip_off(self):
        # Check PIP setting is off
        pipstatus = self.adb.executeShellCommand("configtool pipstatus")[0]
        if 'false' in pipstatus:
            self.log.info("PIP is %s" %pipstatus)
        else:
            self.log.info("PIP is %s, try to turn off PIP." %pipstatus)
            response = self.uut_owner.disable_pip()
            self.log.info(response)
            if response.status_code != 200:
                raise self.err.TestFailure("Failed to turn off PIP.")
            else:
                pipstatus = self.adb.executeShellCommand("configtool pipstatus")[0]
    ...
    def sumologic_check_factory_reset_log(self):
        ...
        if self.counter <= 0:
            ...
            self.log.info("Factory Reset Log Uploaded When PIP is Off Test: PASSED")
...
if __name__ == '__main__':
    parser = InputArgumentParser("""\
        *** Factory Reset Log Uploaded When PIP is Off Script ***
        Examples: ./run.sh functional_tests/factory_reset_log_upload_when_pip_off.py --cloud_env qa1 --serial_server_ip 10.0.0.6 --serial_server_port 20000\
        """)

    test = FactoryResetLogUploadWhenPIPIsOff(parser)
    resp = test.main()
