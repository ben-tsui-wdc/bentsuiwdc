...
class windowsDesktopAppDownload(KDPTestCase):
    ...
    def before_test(self):
        ...
        # Remove SMBTestFolders in Windows client.
        cmd = "Remove-Item -Recurse -Force {}:\\{}; ".format(self.windows_dataset_path.split(":")[0], self.windows_client_folder) 
        result = self.XMLRPCclient(cmd)

        self.log.info('Create folder "{}" on Windows client'.format(self.windows_client_folder))
    ...
    def after_loop(self):
        ...
        if self.html_format == '2':
            html_inst.table_title_column = ['product', 'build', 'RestSDK', 'iteration', 'TotalBytes', 'TotalRealFiles', 'DownAvgSpd','TargetDownAvgSpd','Wifi','DesktopApp']
        ...
        csv_inst = CsvFormat()
        csv_inst.csv_title_column = ['product', 'Direction', 'Client', "Protocol", "FileType", 'TotalRealFiles', 'TotalBytes', 'build', 'DownAvgSpd', 'TransferRate_unit', 'count', 'executionTime','Wifi','DesktopApp']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--uut_share', help='the share folder in target device which could be accessed by SMB', default='desktopapp_download')
    ...
    test = windowsDesktopAppDownload(parser)
    if test.main():
        sys.exit(0)
    sys.exit(1)
