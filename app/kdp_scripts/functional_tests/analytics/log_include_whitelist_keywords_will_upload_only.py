# -*- coding: utf-8 -*-
""" Case to check only the logs include whitelist keywords will be uploaded
"""
...
class LogIncludeWhitelistKeywordsWillUploadOnly(KDPTestCase):

    TEST_SUITE = 'KDP_Functional_Analytics'
    TEST_NAME = 'KDP-1645 [ANALYTICS] Only the logs include keywords in whitelist will be uploaded'
    TEST_JIRA_ID = 'KDP-1645'
    ...
    def init(self):
        ...
        self.non_white_list_keyword = 'dummy_log_test'
    ...
    def test(self):
        self.log.info("*** Step 1: Add the keyword to white list")
        ...
        self.ssh_client.generate_logs(log_number=self.log_number, log_messages=self.white_list_keyword)
        self.ssh_client.generate_logs(log_number=self.log_number, log_messages=self.non_white_list_keyword)
        ...
        return_code, output = self.ssh_client.execute('grep -r "{0}" {1}'.
                                                      format(self.non_white_list_keyword, self.uploaded_log_path))
        if return_code != 1:
            raise self.err.TestFailure('The non white list keyword should not be found in uploaded folder! '
                                       'Test failed! Output: {}'.format(output))

        return_code, output = self.ssh_client.execute('grep -r "{0}" {1}'.
                                                      format(self.white_list_keyword, self.uploaded_log_path))
        if return_code != 0:
            raise self.err.TestFailure('The white list keyword should have been found in uploaded folder! '
                                       'Test failed! Output: {}'.format(output))
        
    def after_test(self):
        self.log.info("*** After test step 1: Clean up the white list keyword in the policy file")
...
if __name__ == '__main__':
    parser = KDPInputArgumentParser("""\
        *** Unauthenticalted ports check script ***
        Examples: ./run.sh kdp_scripts/functional_tests/analytics/log_include_whitelist_keywords_will_upload_only.py --uut_ip 10.92.224.68\
        """)

    test = LogIncludeWhitelistKeywordsWillUploadOnly(parser)
    resp = test.main()
