# -*- coding: utf-8 -*-
""" Case to check the logs include blacklist keywords will not upload
"""
...
class LogIncludeBlacklistKeywordsWillNotUpload(KDPTestCase):

    TEST_SUITE = 'KDP_Functional_Analytics'
    TEST_NAME = 'KDP-1644 - [ANALYTICS] Logs include keywords in blacklist will not be uploaded'
    TEST_JIRA_ID = 'KDP-1644'
    ...
    def test(self):
        self.log.info("*** Step 1: Add the keyword to black list")
        ...
        self.ssh_client.generate_logs(log_number=self.log_number, log_messages=self.black_list_keyword)
        ...
        return_code, output = self.ssh_client.execute('grep -r "{0}" {1}'.
                                                      format(self.black_list_keyword, self.uploaded_log_path))
        if return_code != 1:
...
if __name__ == '__main__':
    parser = KDPInputArgumentParser("""\
        *** Unauthenticalted ports check script ***
        Examples: ./run.sh kdp_scripts/functional_tests/analytics/log_include_blacklist_keywords_will_not_upload.py --uut_ip 10.92.224.68\
        """)

    test = LogIncludeBlacklistKeywordsWillNotUpload(parser)
    resp = test.main()
