...
class CheckNormalLogRotate(KDPTestCase):

    TEST_SUITE = 'KDP_Functional_Analytics'
    TEST_NAME = 'KDP-236 - [ANALYTICS] Logs will be rotated every 15 minutes when the log size reaches 400 KB'
    TEST_JIRA_ID = 'KDP-236'
    ...
    def test(self):
        ...
        self.log.info("*** Step 2: Generate a fake log file with size more than 400 KB")
        # It will take too long if we generate real logs to reach 400 KB
        self.ssh_client.execute_cmd("dd if=/dev/zero of={} bs=1K count=410".format(self.test_log_file))
        ...
        if log_file_size < 400:
            raise self.err.TestSkipped('Log file size should not less than 400 KB after generating fake logs!')
        ...
        if self.ssh_client.check_file_in_device("{}.1".format(self.test_log_file)):
            self.log.info("Logs are rotated when file size >= 400 KB, test passed!")
        else:
            raise self.err.TestFailure('Logs was not rotated when file size >= 400 KB!')


if __name__ == '__main__':
    parser = KDPInputArgumentParser("""\
        *** Unauthenticalted ports check script ***
        Examples: ./run.sh kdp_scripts/functional_tests/analytics/check_normal_log_rotate.py --uut_ip 10.92.224.68\
        """)

    test = CheckNormalLogRotate(parser)
    resp = test.main()
