...
class GetVideoStreamTest(GetDataByIDTest):
    ...
    TEST_NAME = 'GET Video Stream'
    # Popcorn
    TEST_TYPE = 'functional'
    TEST_JIRA_ID = 'KAM-20139'
    PRIORITY = 'blocker'
    COMPONENT = 'REST SDK'
    ISSUE_JIRA_ID = None
    ...
    def test(self):
        if not self.adb.wait_all_FFmpeg_finish(delay=10, timeout=60*60):
    ...
    def verify_content(self, video_format='Matroska', video_codec=None, video_resolution=None, video_width=None,
            video_height=None):
        file_info = MediaInfo(filename=self.local_file, shell_exe=self.adb.executeCommand)
